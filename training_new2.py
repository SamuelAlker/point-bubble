...
def main():
    ...
    val_datas_mae = tf.data.Dataset.from_tensor_slices((data_mae[0][-1000:], data_mae[1][-1000:])).shuffle(
        buffer_size=20000).batch(batch_size=32, drop_remainder=True)
    datas_mae = tf.data.Dataset.from_tensor_slices((data_mae[0][:-1000], data_mae[1][:-1000])).shuffle(
        buffer_size=20000).batch(batch_size=32, drop_remainder=True)
    ...
    for i in range(0, 16):
        ...
        prediction_gif(model_mae, data_mae, min_c_mae, max_c_mae, min_c_mae_1, max_c_mae_1,
